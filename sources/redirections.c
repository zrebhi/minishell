...
/*   Updated: 2023/03/31 15:35:39 by zrebhi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_redirect_infile(char **parsed_line, t_cmdlist *cmds, int i)
{
	if (cmds->infile && close(cmds->infile) == -1)
		perror("close infile here");
	if (!ft_error(parsed_line, i) && !cmds->error)
	{
		cmds->infile = open(ft_remove_quote_in_string\
		(parsed_line[i + 1], 0, 0), O_RDONLY);
		if (cmds->infile == -1)
		{
			perror("open infile");
			cmds->error = 1;
...
void	ft_redirect_outfile(char **parsed_line, t_cmdlist *cmds, int i)
{
	...
	if (!ft_error(parsed_line, i) && !cmds->error)
	{
		cmds->outfile = open(ft_remove_quote_in_string\
		(parsed_line[i + 1], 0, 0), \
...
void	ft_redirect_outfile_append(char **parsed_line, t_cmdlist *cmds, int i)
{
	...
	{
		cmds->outfile = open(ft_remove_quote_in_string\
		(parsed_line[i + 1], 0, 0), O_WRONLY | O_CREAT | \
				O_APPEND, 0644);
		if (cmds->outfile == -1)
		{
			perror("open outfile");
